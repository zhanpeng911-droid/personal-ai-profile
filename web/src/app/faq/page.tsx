  ...
  return (
    <div className="mx-auto max-w-3xl space-y-8">
      <Reveal>
        ...
        <h1 className="mt-2 font-display text-4xl font-bold text-ink-900 sm:text-5xl">精选问答</h1>
      ...
      <div className="space-y-3">
        {faq.map((item, i) => (
          <Reveal key={item.q} delay={i * 60}>
            <details className="group glass-hover p-5">
              <summary className="flex cursor-pointer list-none items-center justify-between gap-3 font-display text-base font-medium text-ink-900">
                <span>{item.q}</span>
                <span className="inline-flex h-6 w-6 shrink-0 items-center justify-center glass-tag text-ink-400 transition-all duration-300 group-open:rotate-180 group-open:border-clay/30 group-open:bg-clay/5 group-open:text-clay">
                  <ChevronDown size={14} />
                </span>
              </summary>
